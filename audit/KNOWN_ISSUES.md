...
| 编号 | 待办 | 状态 |
|---|---|---|
| P1 | Git 历史作者信息匿名化（用户已批准改写历史） | 待执行（filter-repo，需重写全部 29 个提交） |
| P2 | 环境 ID `YOUR_ENV_ID` 占位符化（用户已批准） | 待执行（与构建注入配置配套） |
| P3 | `keystore.properties.example` / `cloudbase.local.properties.example` 模板 | 待生成 |
| P4 | README / LICENSE / NOTICE / SECURITY.md / 贡献指南 / 隐私政策 | 待生成（阶段八） |
| P5 | 更新清单 URL 与 provisioning 文档中的托管地址随 P2 同步处理 | 待执行 |
